...
- [ ] **Step 8: Actualizar `.env`**

```bash
cd /c/Users/DELL/Desktop/code/herramientas/tess
grep -nE "MODEL_PROVIDER|MODEL_NAME|MODEL_API_KEY|EMBEDDING_|AI_GATEWAY" .env
```

Editar `.env`: poner `MODEL_PROVIDER=fake` y `EMBEDDING_PROVIDER=fake`, borrar `MODEL_NAME`, `MODEL_API_KEY`, `EMBEDDING_API_KEY`, `EMBEDDING_MODEL` y `AI_GATEWAY_API_KEY`, y añadir `OPENROUTER_CHAT_MODEL` y `OPENROUTER_EMBEDDING_MODEL` con los valores fijados en la Tarea 1.

- [ ] **Step 9: Commit**

```bash
cd /c/Users/DELL/Desktop/code/herramientas/tess
git add services/api/src/env.ts services/api/src/env.test.ts services/ingest-worker/src/env.ts services/ingest-worker/src/env.test.ts services/ingest-worker/vitest.config.ts
