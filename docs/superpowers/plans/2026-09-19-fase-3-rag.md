...
### Task 14: `ModelProvider` se ensancha y el AI SDK se va
...
En la misma tarea desaparecen `ai` y `@ai-sdk/gateway`: `model-provider.gateway.ts` era su único consumidor.
...
- Produces:
  ...
  - `createModelProvider(env: TessEnv): ModelProvider` pasa a devolver `fake` u `openrouter`.
  - Los consumen las Tareas 16 y 20.

- [ ] **Step 1: Comprobar quién usa el AI SDK antes de quitarlo**
...
Esperado: solo `services/api/src/agent/model-provider.gateway.ts`, `services/api/package.json` y el catálogo de `pnpm-workspace.yaml`. Si aparece algún consumidor más, **detenerse y replantear**: el spec dio por hecho que el gateway era el único.
...
En este punto el typecheck falla: `model-provider.openrouter.js` no existe todavía. Se crea en la Tarea 16, y hasta entonces el paquete no compila. Es deliberado — el commit de esta tarea y el de la 16 se verifican juntos en la 16.

- [ ] **Step 3: Borrar el proveedor del gateway**
...
- [ ] **Step 4: Quitar las dependencias del AI SDK**
...
- [ ] **Step 5: Comprobar que se fueron de verdad**
...
```bash
cd /c/Users/DELL/Desktop/code/herramientas/tess
git add services/api/src/agent/model-provider.ts services/api/package.json pnpm-workspace.yaml pnpm-lock.yaml
git add -A services/api/src/agent/
git commit -m "refactor(api): ensanchar ModelProvider con metadata y retirar el AI SDK

onMetadata es opcional: el provider fake no cambia. ai y @ai-sdk/gateway se
van porque su unico consumidor era el proveedor del Gateway."
```
...
- Consumes: `ModelProvider`, `ModelStreamInput`, `ModelCallMetadata` de la Tarea 14; `leerStreamOpenRouter` de la Tarea 15.
- Produces: `createOpenRouterModelProvider(options: { apiKey: string; model: string; referer?: string; appTitle?: string; fetchImpl?: typeof fetch }): ModelProvider`. La consume `createModelProvider` de la Tarea 14.
...
- [ ] **Step 4: Verificar el paquete entero**

Aquí se cierra lo que la Tarea 14 dejó sin compilar.
...
Esperado: los tres en verde. `typecheck` es el que confirma que `createModelProvider` de la Tarea 14 ya resuelve su import.

- [ ] **Step 5: Commit**

```bash
cd /c/Users/DELL/Desktop/code/herramientas/tess
git add services/api/src/agent/model-provider.openrouter.ts services/api/src/agent/model-provider.openrouter.test.ts
git commit -m "feat(api): chat por REST contra OpenRouter

Politica de proveedor restrictiva por defecto: data_collection deny y sin
fallbacks. El prompt nunca viaja en el mensaje de error."
```
...
for (const archivo of archivosTs('services/api/src')) {
  ...
  for (const prohibido of PROHIBIDOS) {
    if (
      contenido.includes(`from '${prohibido}`) ||
      contenido.includes(`/${prohibido}`)
    ) {
      if (contenido.includes(`import`) && contenido.includes(prohibido)) {
        fallar(
          `${archivo} referencia '${prohibido}': la ingesta debe vivir solo en el worker`,
        );
      }
    }
  }
