...
- [ ] **Step 4: Verificar los tests del API**
...
- [ ] **Step 5: Crear el entorno del worker**
...
- [ ] **Step 6: Dar al worker un runner de tests**
...
- [ ] **Step 8: Actualizar `.env` y `.env.example`**
 ...
 * Selector por entorno.
 *
 * Sigue siendo el de F2. La Tarea 16 lo cambia a OpenRouter, cuando exista el
 * proveedor al que cambiarse.
 */
...
**Ojo con el orden respecto a la Tarea 6.** Si la Tarea 6 ya corrió, `TessEnv` no tiene `MODEL_NAME` ni `AI_GATEWAY_API_KEY` y el selector de arriba no compila. En ese caso —que es el esperado, porque la 6 va antes— dejar el selector así:

```ts
export function createModelProvider(env: TessEnv): ModelProvider {
  // La Tarea 16 añade aquí la rama de OpenRouter. Mientras tanto `fake` es el
  // único proveedor cableado, que es lo que CI usa de todos modos.
  return createFakeModelProvider();
}
```

y borrar el import de `createGatewayModelProvider`. El archivo `model-provider.gateway.ts` se queda en disco sin usar hasta que la Tarea 16 lo borre; eso no rompe el build.
